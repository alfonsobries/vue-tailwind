...
describe('TTable.vue', () => {
  ...
  it('renders all the data attributes attributes when are objects', () => {
    ...
    data.forEach((row, r) => {
      Object.keys(row).forEach((attrib, c) => {
        expect(wrapper.findAll('table > tbody > tr').at(r).findAll('td').at(c).text()).toBe(String(row[attrib]))
      })
    })
  })
  ...
  it('renders the column using the column slot', () => {
    ...
    data.forEach((row, r) => {
      Object.keys(row).forEach((attrib, c) => {
        expect(wrapper.findAll('table > tbody > tr').at(r).findAll('td').at(c).text()).toBe(`${r}:${attrib}:${row[attrib]}`)
      })
    })
  })
  ...
  it('renders the row using the row slot', () => {
    ...
    expect(wrapper.findAll('table > tbody > tr').at(0).findAll('td').at(0).attributes('colspan')).toBe("4")
    
    data.forEach((row, r) => {
      expect(wrapper.findAll('table > tbody > tr').at(r).findAll('td').at(0).text())
